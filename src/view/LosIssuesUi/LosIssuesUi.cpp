...
void LosIssuesUi::updateTable(const QString &file_path,
  ...
  for (int i = 0; i < diags.size(); i++) {
    ...
    QTableWidgetItem *item_3 =
        new QTableWidgetItem(QString::number(diag.startLine + 1));
    item_3->setData(Qt::UserRole, diag.startLine);
    QTableWidgetItem *item_4 =
        new QTableWidgetItem(LosModel::LosFilePath(file_path).getFileName());
...
void LosIssuesUi::onTableDoubleClicked(int row, int colume) {
  ...
  emit _gotoFile(targetFile, targetLine);
}
...
void LosIssuesUi::initUi() {
  ...
  L_table->horizontalHeader()->setSectionResizeMode(
...
void LosIssuesUi::initConnect() {
  connect(L_table, &QTableWidget::cellDoubleClicked, this,
          &LosIssuesUi::onTableDoubleClicked);
}
