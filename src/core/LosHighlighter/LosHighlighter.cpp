...
void LosHighlighter::highlightBlock(const QString &str) {
  ...
  while (startIndex >= 0) {
    ...
    QRegularExpressionMatch match =
        L_commentEndExpression.match(str, startIndex);
    ...
    // /*1*/
    startIndex =
        str.indexOf(L_commentStartExpression, startIndex + commentLenth);
  }
}
...
void LosHighlighter::initRule() {
  ...
  rule.L_regex =
      QRegularExpression(QStringLiteral("\\b[Q]?[A-Z][a-zA-Z0-9_]+\\b"));
