...
- 带 id 的
*/
void LosLspClient::sendRequestForCompletion(const QString &file_path, int line, int character)
{
...
{
    connect(L_process, &QProcess::started, this, [=]() { sendInitializeRequest(); });
...
void LosLspClient::dealLspMessage(const QJsonObject &obj)
{
    if (obj.contains("id"))
    {
        ...
        {
            qDebug() << "[lsp] handshake successful, sending initialized confirmation";
            sendInitializedMsg();
            break;
        }
